...
const posts = [
  { slug: "new", data: { date: new Date("2025-01-01T01:00:00Z"), categories: "a, b" } },
  { slug: "old", data: { date: new Date("2024-12-31T10:00:00Z"), categories: "a" } },
] as Post[];

test("blog views keep ordering, categories, routes, and publication dates aligned", () => {
  const blog = createBlogCatalog(posts);

  expect(blog.recent(1).map((post) => post.slug)).toEqual(["new"]);
  expect(blog.archiveByYear().map(({ year, posts }) => [year, posts.map((post) => post.slug)])).toEqual([
    [2025, ["new"]],
    [2024, ["old"]],
  ]);
  expect(blog.categoryNames()).toEqual(["a", "b"]);
  expect(blog.categoryPaths().map(({ params }) => params.category)).toEqual(["a", "b"]);
  expect(blog.categoryPaths()[0].props.posts.map((post) => post.slug)).toEqual(["new", "old"]);
  ...
  expect(blog.feedPosts().map((post) => post.slug)).toEqual(["new", "old"]);
