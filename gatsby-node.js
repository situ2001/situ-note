exports.createPages = async ({ actions, graphql }) => {
  ...
  const postList = require.resolve(
    `${__dirname}/src/templates/paginated-blog-post-list.tsx`,
  );

  const {
    data: {
      site: {
        siteMetadata: { dateFormat },
      },
    },
  } = await graphql(`
    {
  ...
  data.allMdx.nodes.forEach((node, i) => {
    ...
    if (i > 0) {
      ...
      prev.to = `/blog/${
        prevNode.frontmatter.permalink ?? prevNode.parent.name ?? null
      }`;
    }
    ...
    if (i < postCount - 1) {
      ...
      next.to = `/blog/${
        nextNode.frontmatter.permalink ?? nextNode.parent.name ?? null
      }`;
    }
